...
// ----------- Materials ------------
uniform sampler2D	texGroundSampler;
uniform sampler2D	texSnowSampler;
uniform sampler2D	texRockSampler;
uniform float		groundTextureScale;
uniform float		snowHeightFactor;
uniform float		snowHeightThreshold;
uniform float		snowSlopeFactor;
uniform float		snowTextureScale;
uniform float		rockTextureScale;
uniform float		rockWeightFactor;
...
uniform int				minNeighborhood, maxNeighborhood;							
...
vec3 getDiffuse(const vec3 fragKad, const float dotLN)   // TODO
{
...
// Obtains color of terrain depending on its slope
vec4 getTerrainKad(vec3 fragNormal)
{
	vec4 groundColor = texture(texGroundSampler, textCoord * groundTextureScale);
	vec4 rockColor = texture(texRockSampler, textCoord * rockTextureScale);
	vec4 snowColor = texture(texSnowSampler, textCoord * snowTextureScale);
	const float rockWeight = (abs(dot(fragNormal, vec3(1.0f, 0.0f, 0.0f))) + abs(dot(fragNormal, vec3(0.0f, 1.0f, 0.0f)))) * rockWeightFactor + 0.2f;
	float relativeHeight = (worldPosition.y - heightBoundaries.x) / (heightBoundaries.y - heightBoundaries.x);
	float snowWeight = clamp(smoothstep(snowHeightThreshold, 1.0f, relativeHeight) * snowHeightFactor - clamp(1.0f - (dot(fragNormal, vec3(.0f, 1.0f, 0.0f)) + 1), .0f, 1.0f) * snowSlopeFactor, .0f, 1.0f);

	return vec4(mix(mix(groundColor.rgb, rockColor.rgb, rockWeight), snowColor.rgb, snowWeight), groundColor.w);
}
...
{
	return normalize(vec3((2.0f * texture(texBumpSampler, textCoord)) - 1.0f));
}
...
void main()
{
	...
	const vec4 fragKad = getTerrainKad(fragNormal);

	float shadowDiffuseFactor, shadowSpecFactor;
	depthTextureUniform(shadowDiffuseFactor, shadowSpecFactor);

	if (textCoord.x < .0f && textCoord.y < .0f) shadowDiffuseFactor = shadowSpecFactor = 1.0f;
