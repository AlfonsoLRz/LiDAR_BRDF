...
const float InputManager::SCENE_MULTIPLIER = 0.1f;
...
float InputManager::getSceneMovementMultiplier(LiDARScene* scene)
{
	return std::max(scene->getAABB().size().x, std::max(scene->getAABB().size().y, scene->getAABB().size().z)) * SCENE_MULTIPLIER;
}
...
bool InputManager::processPressedKeyEvent(const int key, const int mods)
{
	...
	case InputManager::DOLLY_POS_KEY:
		...
		{
			camera->dolly(MOVEMENT_SPEED[DOLLY] * getSceneMovementMultiplier(renderer->getCurrentScene()) + _movementsInARow[DOLLY] * MOVEMENT_SPEED[DOLLY] * MOVEMENT_MULTIPLIER);
	...
	case InputManager::TRUCK_POS_KEY:
		if (_rightClickPressed)
		{
			camera->truck(MOVEMENT_SPEED[TRUCK] * getSceneMovementMultiplier(renderer->getCurrentScene()) + _movementsInARow[TRUCK] * MOVEMENT_SPEED[TRUCK] * MOVEMENT_MULTIPLIER);
	...
	case InputManager::BOOM_KEY:
		camera->boom(MOVEMENT_SPEED[BOOM] * getSceneMovementMultiplier(renderer->getCurrentScene()));

		break;

	case InputManager::CRANE_KEY:
		camera->crane(MOVEMENT_SPEED[BOOM] * getSceneMovementMultiplier(renderer->getCurrentScene()));
