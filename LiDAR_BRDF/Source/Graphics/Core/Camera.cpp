...
const float Camera::FOV_X				= 90.0f * glm::pi<float>() / 180.0f;
...
void Camera::debugLiDARTracking()
{
#if LIDAR_TRACKING
	if (glm::distance(_eye, _lastPosition) > TRACKING_DISTANCE)
	{
		std::cout << _eye.x << ", " << _eye.y << ", " << _eye.z << std::endl;
		_lastPosition = _eye;
	}
#endif
}
