...
public:
	...
	*/
	...
	/**
	*	@return Color linked to an ASPRS semantic group.
	*/
	vec3 getASPRSColor(unsigned int group);
	...
	*	@brief 
	*/
	unsigned getNumTriangles() { if (_staticGPUData) return _staticGPUData->_numTriangles; else return 0; }
	...
	*/
	...
	/**
	*	@return Color linked to a semantic group.
	*/
	vec3 getSemanticColor(unsigned int group);
	...
	*/
	...
	/**
	*	@brief Renders the points captured by a sensor. Colors depend on the instance they belong to.
	*	@param shader Shader which will draw the model.
	*	@param shaderType Index of shader at the list, so we can identify what type of uniform variables must be declared.
	*	@param matrix Vector of matrices which can be applied in the rendering process.
	*/
	...
	/**
	*	@brief Renders the points captured by a sensor. Colors depend on the RGB shading.
	*	@param shader Shader which will draw the model.
	*	@param shaderType Index of shader at the list, so we can identify what type of uniform variables must be declared.
	*	@param matrix Vector of matrices which can be applied in the rendering process.
	*/
