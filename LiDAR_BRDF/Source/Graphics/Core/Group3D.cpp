...
vec3 Group3D::getASPRSColor(unsigned int group)
{
	return _asprsGroupColor[group];
}

void Group3D::getASPRSSemanticGroup(unsigned modelCompID, int& lastSemanticGroup, bool& found)
{
	for (Model3D* object : _objects)
	{
		if (found) break;
		if (_modelComp[0]->_asprsSemanticGroup >= 0) lastSemanticGroup = _modelComp[0]->_asprsSemanticGroup;

		object->getASPRSSemanticGroup(modelCompID, lastSemanticGroup, found);
	}
}
...
vec3 Group3D::getSemanticColor(unsigned int group)
{
	return _groupColor[group];
}

void Group3D::getSemanticGroup(unsigned modelCompID, int& lastSemanticGroup, bool& found)
{
	for (Model3D* object : _objects)
	{
		if (found) break;
		if (_modelComp[0]->_semanticGroup >= 0) lastSemanticGroup = _modelComp[0]->_semanticGroup;

		object->getSemanticGroup(modelCompID, lastSemanticGroup, found);
	}
}
...
bool Group3D::load(const mat4& modelMatrix)
{
	...
	for (int modelIdx = 0; modelIdx < _objects.size(); ++modelIdx)
	{
		success &= _objects[modelIdx]->load(mMatrix);
	}
...
void Group3D::retrieveColorsGPU()
{
	for (Model3D* object : _objects)
	{
		object->retrieveColorsGPU();
	}
}
...
void Group3D::drawAsTrianglesWithGroup(RenderingShader* shader, const RendEnum::RendShaderTypes shaderType, std::vector<mat4>& matrix, bool ASPRS)
{
	this->setShaderUniforms(shader, shaderType, matrix);

	for (Model3D* object : _objects)
	{
		object->drawAsTrianglesWithGroup(shader, shaderType, matrix, ASPRS);
	}
}
...
void Group3D::drawCapturedPointsInstance(RenderingShader* shader, const RendEnum::RendShaderTypes shaderType, std::vector<mat4>& matrix)
{
	this->setShaderUniforms(shader, shaderType, matrix);

	for (Model3D* object : _objects)
	{
		object->drawCapturedPointsInstance(shader, shaderType, matrix);
	}
}

void Group3D::drawCapturedPointsRGB(RenderingShader* shader, const RendEnum::RendShaderTypes shaderType, std::vector<mat4>& matrix)
{
	this->setShaderUniforms(shader, shaderType, matrix);

	for (Model3D* object : _objects)
	{
		object->drawCapturedPointsRGB(shader, shaderType, matrix);
	}
}

void Group3D::drawReflectiveCapturedPoints(RenderingShader* shader, const RendEnum::RendShaderTypes shaderType, std::vector<mat4>& matrix, Camera* camera, const GLuint lightIndex)
{
	this->setShaderUniforms(shader, shaderType, matrix);

	for (Model3D* object : _objects)
	{
		object->drawReflectiveCapturedPoints(shader, shaderType, matrix, camera, lightIndex);
	}
}

void Group3D::drawCapturedPointsWithGroups(RenderingShader* shader, const RendEnum::RendShaderTypes shaderType, std::vector<mat4>& matrix, bool ASPRS)
{
	for (Model3D* object : _objects)
	{
		if (!ASPRS && _modelComp[0]->_semanticGroup != -1) shader->setUniform("vColor", _groupColor[_modelComp[0]->_semanticGroup]);
		else if (ASPRS && _modelComp[0]->_asprsSemanticGroup != -1) shader->setUniform("vColor", _asprsGroupColor[_modelComp[0]->_asprsSemanticGroup]);

		object->drawCapturedPointsWithGroups(shader, shaderType, matrix, ASPRS);
	}

	shader->setUniform("vColor", vec3(.0f));			// Reset color
}
...
void Group3D::aggregateSSBOData(VolatileGPUData*& volatileGPUData, StaticGPUData*& staticGPUData)
{
	...
	std::cout << "Number of vertices: " << groupData->_geometry.size() << std::endl;
	std::cout << "Number of triangles: " << groupData->_triangleMesh.size() << std::endl;
	...
	for (ModelComponent* modelComp : _globalModelComp)
	{
		...
		groupData->_meshData[modelComp->_id]._materialID = modelComp->_materialID;
	...
	std::cout << "AABB Size: " << this->_aabb.size().x << ", " << this->_aabb.size().y << ", " << this->_aabb.size().z << std::endl;
	std::cout << "AABB Center: " << this->_aabb.center().x << ", " << this->_aabb.center().y << ", " << this->_aabb.center().z << std::endl;
...
void Group3D::buildClusterBuffer(VolatileGPUData* gpuData, const GLuint sortedFaces)
{
	...
	_staticGPUData->_clusterSSBO	= ComputeShader::setWriteBuffer(BVHCluster(), clusterSize, GL_DYNAMIC_DRAW);
