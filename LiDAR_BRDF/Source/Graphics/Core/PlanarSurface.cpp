...
void PlanarSurface::drawAsTrianglesWithGroup(RenderingShader* shader, const RendEnum::RendShaderTypes shaderType, std::vector<mat4>& matrix, bool ASPRS)
{
	this->renderTrianglesWithGroup(shader, shaderType, matrix, _modelComp[0], GL_TRIANGLE_STRIP, ASPRS);
}
