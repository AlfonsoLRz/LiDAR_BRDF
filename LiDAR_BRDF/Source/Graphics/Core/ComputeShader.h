...
public:	
	...
	*/
	...
	/**
	*	@return Maximum number of rays allowed in a single buffer in GPU.
	*/
	template<typename T>
...
template<typename T>
inline unsigned ComputeShader::getAllowedNumberOfInstances(T data)
{
	unsigned raySize = sizeof(data);
	GLint limitedMemory;
	glGetIntegerv(GL_MAX_SHADER_STORAGE_BLOCK_SIZE, &limitedMemory);

	return std::floor(limitedMemory / raySize);
}
