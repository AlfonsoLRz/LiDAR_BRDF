...
void Scene::drawAsTrianglesWithGroups(const mat4& mModel, RenderingParameters* rendParams)
{
	Camera* activeCamera = _cameraManager->getActiveCamera(); if (!activeCamera) return;
...
void Scene::bindDefaultFramebuffer(RenderingParameters* rendParams)
{
	...
	glClearColor(rendParams->_backgroundColor.x, rendParams->_backgroundColor.y, rendParams->_backgroundColor.z, _nextFramebufferID ? rendParams->_screenshotAlpha : 1.0f);
