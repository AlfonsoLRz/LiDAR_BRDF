...
CatmullRom::CatmullRom(std::vector<vec2>& waypoints, float height): Interpolation(waypoints, height)
{
}
...
LiDARPath CatmullRom::getLiDARPath()
{
    LiDARPath path;
    for (vec4& point : _waypoints)
    {
        path.push_back(vec2(point.x, point.z));
    }
...
{
    if (_waypoints.empty()) return vec4(.0f);
