...
public:
	...
	*	@param waypoints Main point which conform interpolation shape.
	*/
	Interpolation(std::vector<vec2>& waypoints, float height);
	...
	*/
	...
	/**
	*	@brief Retrieves point by sampling with a frequency of t.
	*/
	void getPoints(std::vector<vec4>& points, float t);
